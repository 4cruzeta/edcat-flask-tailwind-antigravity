...
## 15. Estabilização de Agentes: Armadilhas e Soluções (Abril 2026)
A fase final de integração do `g_calendar_agent` revelou desafios profundos sobre a natureza dos modelos LLM "Lite" (como o Gemini 2.5 Flash Lite) e a sensibilidade do Python com fusos horários. Documentamos aqui as "pegadinhas" que consumiram horas de depuração:

### 15.1. O Fenômeno do "Falso Sucesso" (Hallucination)
*   **O Problema**: O Agente informava ao usuário que o horário estava "marcado com sucesso" sem nunca ter disparado a ferramenta `confirm_booking_tool`. Isso ocorria porque o modelo Flash tentava ser "prestativo" e simulava o fluxo final para encerrar a conversa.
*   **A Solução**: Implementamos uma **Trava de Segurança no Prompt**. O Agente foi terminantemente proibido de emitir a mensagem de sucesso a menos que recebesse um identificador real (ID) retornado pela ferramenta na mesma rodada de execução.

### 15.2. A Guerra dos Fusos (UTC vs America/Sao_Paulo)
*   **O Problema**: O sistema sofria com um "salto" de 3 horas. O usuário pedia 11h, a IA entendia 11h, mas o Google Calendar registrava 08h.
*   **A Causa**: A IA recebia horários em UTC (`Z`) e tentava "normalizá-los" por conta própria, confundindo a ferramenta de backend. Além disso, o Python lançava erros fatais de `TypeError` ao tentar comparar datas com fuso (**Aware**) e sem fuso (**Naive**).
*   **A Solução (Estratégia do Horário Naive)**: 
    1. Passamos a enviar para a IA apenas strings de tempo "puras" (ex: `11:00:00`), sem sufixos de fuso.
    2. No backend, a função de agendamento assume a responsabilidade única de "carimbar" o fuso `America/Sao_Paulo` sobre essa hora pura antes de enviar ao Google.

### 15.3. A Estratégia do Mapa de Slots Invisível (`MAPA_DE_SLOTS_UTF8`)
*   **O Problema**: IAs frequentemente erram a conversão de "Segunda-feira" para uma data ISO real (ex: `2026-04-13`).
*   **A Solução**: Criamos um dicionário JSON oculto que viaja na mensagem da ferramenta. O usuário vê uma tabela amigável, mas a IA lê um mapa técnico. Isso garante que o Agente use o ISO Exato gerado pelo servidor, eliminando erros de calendário "inventado".

### 15.4. O Perigo da "Faxina Prematura"
*   **Lição de Workflow**: Durante a busca pela elegância do código, removemos os `print()` de auditoria (`AI-THINK` e `TOOL-RESULT`) antes do sistema estar 100% estável. Isso resultou em um "vôo às cegas", onde não sabíamos se o erro era na IA ou no servidor.
*   **Regra de Ouro**: Logs de auditoria visual no terminal permanecem ativos até a fase de homologação final. O silêncio só é permitido em produção estável.

---
*Status Atual*: Agente de Calendário Estabilizado, Documentado e à Prova de Alucinações.
